...
#include "softSPI.h"
//--------------P5.0---------------------------------
#define	LCD_CS_SET		SPI_CS(1)
#define 	LCD_CS_CLR		SPI_CS(0)
//--------------P5.2---------------------------------
#define 	LCD_RS_SET		P5OUT |= BIT2
#define 	LCD_RS_CLR		P5OUT &=~BIT2
//--------------P1.1---------------------------------
#define 	LCD_RST_SET		P1OUT |= BIT1
#define 	LCD_RST_CLR		P1OUT &=~BIT1
...
{
  SPI_SendByte ((val >> 8) & 0xFF); //发送高位
  SPI_SendByte(val & 0xFF); //发送低位
}
